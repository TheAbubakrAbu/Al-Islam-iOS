...
1. **Confirm the iCloud capability in Xcode.** Open the project, iPhone target, Signing & Capabilities. iCloud should be listed with CloudKit ticked and the container `iCloud.com.Quran.Elmallah.Islamic-Pillars`. If the container row is red or unticked, tick it (Xcode creates it). **A CloudKit container can never be deleted or renamed**, so this is the one permanent step; the name follows Apple's own default (iCloud. plus the bundle id).
2. **Run a Debug build once on a real iPhone signed in to iCloud**, turn the feature on and make a profile. That first save creates the `Profile` record type in the container's development schema.
3. **Deploy the schema to production before the release goes to TestFlight or the App Store.** CloudKit Console, the container, Schema, "Deploy Schema Changes". TestFlight and App Store builds talk to the production environment, and until the schema is deployed every save there fails. This is the step that cannot be caught on a simulator.
4. **Sibling apps** (Al-Adhan, Al-Quran) need their own pass when this syncs across: see section 11.
...
- **Container:** `CKContainer.default()`. The code never names the container; the entitlement does, as `iCloud.$(CFBundleIdentifier)`, so each sibling app resolves to its own container with no source change.
...
CloudKit itself needs a signed-in iCloud account; the 17 Pro simulator is not signed in, so the network layer is exercised on Abu's device (section 2, step 2).

## 11. Sibling apps

`Resources/` and `iPhone/Settings/` sync to Al-Adhan and Al-Quran, so this feature reaches them on the next `sync_from_islam.sh`.

- The iCloud keys merge into the sibling's main entitlements file, and because the container is `iCloud.$(CFBundleIdentifier)` they are already correct there.
- **Al-Adhan's watch target shares its main entitlements file the same way.** Before its next build: create its own `Entitlements-Watch.entitlements` with **its** app group (`group.com.PrayerTimes.AppGroup`, not the one in the file that syncs across), and repoint its two watch configurations. The synced copy of Al-Islam's watch file carries Al-Islam's group and must be corrected, not used as is.
- Each sibling needs the iCloud capability enabled on its own App ID, its own container, and its own schema deploy (section 2, steps 1 to 3).
- The backup code references no Quran, Hadith or Journal type, and every store joins in by observing a notification, so an app without a store simply has nothing to reload. Manifest keys for features a sibling lacks are harmless: absent keys are never written.
- The root stage lives in the per-app root (`iPhone/Al-<App>App.swift`), which the sync never merges: each sibling's root needs the `.cloudOffer` stage added by hand, and `check_root_wiring.py` should learn to look for it.
- The eight new Settings files (`Cloud{Manifest,Snapshot,MergeRules,BackupManager,BackupViews,BackupDebug,BackupStoreProbe}.swift`, `ContentCategories.swift`) need registering in each sibling's iPhone target; `CloudBackupStoreProbe.swift` names every store and is left OUT of a sibling that lacks one. A sibling's `ContentCategory` table can keep the full list: a category whose keys the app never writes simply reads "None yet".
